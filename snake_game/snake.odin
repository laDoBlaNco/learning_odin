package snake // all files in this package must must use the same name "snake" and all files in one folder should be under the same package to compile.

import rl "vendor:raylib" // here we are using the raylib vendor libraries and note how we  give it an alias 'rl' to use
import "core:math"
import "core:fmt"


// Here we start putting some constants we will use later. We do that with simply a colon, apparently. Didn't know that one
WINDOW_SIZE::900
GRID_WIDTH::20
CELL_SIZE::16
...
place_food::proc(){
  ...
  for x in 0..<GRID_WIDTH{
    for y in 0..<GRID_WIDTH{
      if !occupied[x][y]{
        append(&free_cells,Vec2i{x,y}) // same append from Go
      }
    }
  }
...
restart::proc(){
  // NOTE placing the type here works in both ways (:vec2i={} and :=Vec2i{}) I find that interesting and need to look into it later in the overview.
...
// in Odin programs start with main
main::proc(){
  // we can now open a window using raylib
  ...
  // Then we create the main loop for the window. This will be the main game loop
  // Note that our for loop is infiinite and we used '!' to say, run as long as the window "Shouldn't" close basically
  for !rl.WindowShouldClose(){
    ...
    for i in 0..<snake_length{
      ...
      // rl.DrawTextureV(part_sprite,{f32(snake[i].x),f32(snake[i].y)}*CELL_SIZE,rl.WHITE)
    }
  ...
  // its good practice to unload assets before the programs quits, its not always necessary, but its good to keep a clean house. 
